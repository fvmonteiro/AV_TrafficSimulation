...
VehicleController::VehicleController(const ACCVehicle& acc_vehicle,
...
VehicleController::VehicleController(const AutonomousVehicle& autonomous_vehicle,
	bool verbose) : VehicleController(verbose)
{
	if (verbose)
	{
...
VehicleController::VehicleController(const ConnectedAutonomousVehicle& cav,
	bool verbose) : VehicleController(verbose)
{
	if (verbose)
	{
...
VehicleController::VehicleController(const PlatoonVehicle& platoon_vehicle,
	bool verbose) : VehicleController(verbose)
{
...
void VehicleController::add_lane_change_adjustment_controller(
...
void VehicleController::add_cooperative_lane_change_controller(
...
void VehicleController::update_destination_lane_follower_parameters( 
...
void VehicleController::update_destination_lane_leader_time_headway(
	double time_headway)
{
	if (verbose)
	{
...
void VehicleController::update_gap_generation_controller(double ego_velocity,
	double time_headway)
{
	/* NOTE: include 'if (time_headway_filter.get_is_initialized())' ?
	And force initial value to be the non-lane-changing one? */
...
bool VehicleController::get_destination_lane_desired_acceleration(
	const AutonomousVehicle& autonomous_vehicle,
	std::unordered_map<ALCType, double>& possible_accelerations)
{
	bool is_active = false;
...
bool VehicleController::get_cooperative_desired_acceleration(
	const ConnectedAutonomousVehicle& cav,
	std::unordered_map<ALCType, double>& possible_accelerations)
{
	bool is_active = false;
...
double VehicleController::determine_low_velocity_reference(double ego_velocity,
...
double VehicleController::compute_accepted_lane_change_gap(
...
double VehicleController::compute_accepted_lane_change_gap_exact(
	const EgoVehicle& ego_vehicle, const NearbyVehicle& nearby_vehicle,
